...
namespace big
{
	struct hooks
	{
		...
		static BOOL GetEventData(int32_t eventGroup, int32_t eventIndex, int32_t* argStruct, int32_t argStructSize);

		static BOOL GetTaskEvent(std::uintptr_t thisptr, std::uintptr_t sourcePlayer, std::uintptr_t targetPlayer);
		...
		static BOOL __cdecl Is_DLC_Present();
	...
	private:
		...
		detour_hook m_GetLabelTextHook;
		detour_hook m_GetEventDataHook;
		detour_hook m_GetTaskEventHook;
		detour_hook m_IsDLCPresentHook;
