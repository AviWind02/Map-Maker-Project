...
#include <MinHook.h>
/*
		//void* OG_GetEventData = nullptr;
		//auto handle = reinterpret_cast<decltype(&GetEventData)>(OG_GetEventData)(eventGroup, eventIndex, argStruct, argStructSize);
*/
namespace big
{
	...
	hooking::hooking() :
		...
		m_set_cursor_pos_hook("Set Cursor Pos", memory::module("user32.dll").get_export("SetCursorPos").as<void*>(), &hooks::set_cursor_pos),
		m_run_script_threads_hook("Script hook", g_pointers->m_run_script_threads, &hooks::run_script_threads),
		m_GetLabelTextHook("Get Label Text Hook", g_pointers->m_GetLabelText, &hooks::GetLabelText),
		m_GetEventDataHook("Get Event Data Hook", g_pointers->m_getEventData, &hooks::GetEventData),
		m_GetTaskEventHook("Get Task Event Hook", g_pointers->m_getTaskEvent, &hooks::GetTaskEvent),
		m_IsDLCPresentHook("P Hook", g_pointers->m_IsDLCPresent, &hooks::Is_DLC_Present),
	...
	const char* hooks::GetLabelText(void* unk, const char* label) {
		...
		if (strcmp("PM_SOLO_FM", label) == 0) return "Bob our auth dev is gay (Solo Session)";

		return g_hooking->m_GetLabelTextHook.get_original<decltype(&GetLabelText)>() (unk, label);
	}
	BOOL hooks::GetEventData(int32_t eventGroup, int32_t eventIndex, int32_t* argStruct, int32_t argStructSize)
	{


		return g_hooking->m_GetEventDataHook.get_original<decltype(&GetEventData)>() (eventGroup, eventIndex, argStruct, argStructSize);
	}
	bool protec;
	BOOL hooks::GetTaskEvent(std::uintptr_t thisptr, std::uintptr_t sourcePlayer, std::uintptr_t targetPlayer)
	{
		if (protec) return true;// you are the mean
		return  g_hooking->m_GetTaskEventHook.get_original<decltype(&GetTaskEvent)>()(thisptr, sourcePlayer, targetPlayer);
	}
	BOOL __cdecl hooks::Is_DLC_Present()
	{
		static uint64_t	last = 0;
		uint64_t	cur = g_pointers->m_frameCount;
		if (last != cur)
		{
			last = cur;
			g_script_mgr.tick();
		}
		return g_hooking->m_IsDLCPresentHook.get_original<decltype(&Is_DLC_Present)>()();
	}
	void hooking::enable()
	{
		...
		m_GetEventDataHook.enable();
		m_IsDLCPresentHook.enable();
	...
	void hooking::disable()
	{
		...
		m_convert_thread_to_fiber_hook.disable();
				...
				m_GetEventDataHook.disable();
				m_IsDLCPresentHook.disable();
...
{
	m_worldPtr = (big::g_pointers->m_world) + *reinterpret_cast<int*>((big::g_pointers->m_world) + 3) + 7;
	return m_worldPtr;
}